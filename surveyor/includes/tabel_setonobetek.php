...
<body>
    ...
    $sql = "SELECT DISTINCT
                data_barang_setonobetek.nama_barang,
                data_barang_setonobetek.tanggal,
                data_barang_setonobetek.harga_sekarang
            FROM
    ...
    <div class="p-5 table-responsive">
        <table class="table table-bordered border-dark">
            ...
            <?php
            $selisih = "SELECT harga_kemarin, harga_sekarang FROM data_barang_setonobetek";

            // Check if both 'dari' and 'ke' parameters are set
            if (isset($_GET['dari']) && isset($_GET['ke'])) {
                // Display data within the specified date range
                $data = mysqli_query($conn, "SELECT * FROM data_barang_setonobetek WHERE tanggal BETWEEN '" . $_GET['dari'] . "' AND '" . $_GET['ke'] . "' ORDER BY tanggal DESC LIMIT 42");
            } else {
                // If no date range is specified, display the latest data
                $data = mysqli_query($conn, "SELECT * FROM data_barang_setonobetek ORDER BY tanggal DESC LIMIT 42");
            }

            // Check if 'lokasi' parameter is set
            if (isset($_GET['lokasi'])) {
                // Filter data based on the specified location
                $data = mysqli_query($conn, "SELECT * FROM data_barang_setonobetek WHERE lokasi = '" . $_GET['lokasi'] . "' ORDER BY tanggal DESC LIMIT 42");
            }
